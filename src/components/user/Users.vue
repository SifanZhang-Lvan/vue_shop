    ...
    return {
      ...
      addFormRules: {
        ...
        password: [
          ...
          { min: 6, max: 15, message: '密码的长度在6~16个字符', trigger: 'blur' }
        ],
        email: [
  ...
  methods: {
    ...
    editUserInfo() {
      this.$refs.editFormRef.validate(valid => {
        if (!valid) return
        // 发起修改用户信息的数据请求
        this.$http.put('users/' + this.editForm.id, { email: this.editForm.email, mobile: this.editForm.mobile }).then(({ data: res }) => {
          if (res.meta.status !== 200) {
